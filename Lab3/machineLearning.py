...
lon = 14.826 # Up to you

date = "2013-02-04" # Up to you
...
#(key, value) = (id, ( (date, time, temp), (lat, lon) ) ) 
#temperature = lines_temps.map(lambda x: (x[0], ((x[1], x[2], float(x[3])), (bc.value.get(x[0])))))

#Filter the posterior dates
...
12, 10, 8, 6, 4]:
    ...
    #(id, ( (date, time, temp), (lat, lon) ) ) 
    kernels = temperature.map( lambda x: \
    (   gaussianKernel( dist_time( time, x[1][0][1][0:2] ) , h_time), \
        gaussianKernel( haversine(lon, lat, x[1][1][1], x[1][1][0]), h_distance ), \
    ...
    #Prediction
    prediction_sum.append( kernels_sum[0]/kernels_sum[1] )
    prediction_prod.append( kernels_prod[0]/kernels_prod[1] )
